...
class nitro_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'nitro_rp', // Base ID
			__('Nitro Recent Posts with Thumbnails', 'nitro'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'nitro' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'nitro' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'nitro' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','nitro' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'nitro' ); ?></label>
...
add_action( 'widgets_init', 'register_nitro_widget' );  
function register_nitro_widget() {  
    register_widget( 'nitro_Recent_Posts' );  
}  
